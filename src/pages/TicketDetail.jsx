...
export default function TicketDetail() {
  ...
  const filteredMessages = messages.filter(msg => { 
      if (userRole === 'customer' && msg.is_internal) return false; 
  ...
  return (
    <div className="min-h-screen p-6 font-sans">
      ...
      {/* HEADER BAR - Uses Global Variables */}
        ...
        <div className="flex gap-3 items-center">
            {isEmployee ? (
                <div className="form-control">
                    {/* Select auto-styled by Global CSS */}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-fade">
        <div className="lg:col-span-2 space-y-6">
            ...
            {/* INFO CARD - Uses Semantic 'content-card' */}
            <div className="content-card">
                ...
                <div className="flex justify-between items-center">
                    <div>
                        ...
                        <p className="text-lg text-[var(--text-muted)] font-mono">{ticket.phone}</p>
                    </div>
                    {isEmployee && ( 
            ...
            {/* DETAILS CARD - Uses Semantic 'content-card' */}
            <div className="content-card">
                ...
                {/* Description Box uses 'bg-subtle' for contrast */}
        ...
        <div className="rounded-2xl shadow-xl flex flex-col h-[600px] overflow-hidden border border-[var(--border-color)] bg-[var(--bg-surface)]">
            {isEmployee ? (
                <div role="tablist" className="tabs tabs-lifted bg-[var(--bg-subtle)] p-2">
                    {/* Notes Tab */}
                    <a role="tab" 
                       className={`tab font-bold transition-all flex-1 ${activeTab === 'internal' ? 'tab-active bg-[var(--bg-surface)] text-yellow-600 border-t-2 border-yellow-500' : 'text-slate-400'}`} 
                    ...
                    {/* Chat Tab */}
                    <a role="tab" 
                       className={`tab font-bold transition-all flex-1 ${activeTab === 'public' ? 'tab-active bg-[var(--bg-surface)] text-primary border-t-2 border-primary' : 'text-slate-400'}`} 
            ...
            {/* Chat Body - Background depends on tab */}
                ...
                {filteredMessages.map((msg) => {
                    ...
                    return (
                        <div key={msg.id} className={`chat ${isCustomer ? 'chat-start' : 'chat-end'}`}>
                            ...
                            <div className={`chat-bubble shadow-sm font-medium ${bubbleClass}`}>
                                {msg.message_text}
                            </div>
                        </div>
                    );
            ...
            {/* Chat Input - Matches global 'bg-surface' */}
            <form onSubmit={sendMessage} className="p-4 bg-[var(--bg-surface)] border-t border-[var(--border-color)]">
                <div className="flex gap-2">
                    <input 
                        ...
                        className={`input input-bordered w-full bg-[var(--bg-subtle)] focus:bg-[var(--bg-surface)] transition-all ${activeTab === 'internal' ? 'focus:border-yellow-500' : 'focus:border-primary'}`} 
                        value={newMessage} 
                        onChange={(e) => setNewMessage(e.target.value)} 
                        disabled={isSending}
                    />
                    <button type="submit" className={`btn btn-square shadow-sm ${activeTab === 'internal' ? 'btn-warning text-white' : 'btn-primary text-white'}`} disabled={isSending}>
